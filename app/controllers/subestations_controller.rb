class SubestationsController < ApplicationController
  ...
  before_action :set_zones, only: [:new, :edit]
  ...
  private
    ...
    def subestation_params
      params.require(:subestation).permit(:name, :abreviation, :identifier, :zone_id)
    end

    #Array que contiene las opciones para seleccionar
    def set_zones
      #Creamos un array con todos los nombres de las zonas
      @zones = Array.new

      #Inicializamos nuestro array iterando las zonas para solo seleccionar el campo name
      Zone.all.each do |z|
        @zones.push([z.name, z.id])
      end
    end
end
